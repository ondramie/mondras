...
The Box-Cox procedure performs a data transformation or a regression transformation.  Here, I am only interested in talking about the regression transform, not the data transform.  There are several ways to represent the transform[^1], based on the original distribution of the data, but in generalities, it looks like: 
...
#
```
Based on likelihood profile plot above, the estimate for lambda is `$0.1414$`. Now, I will check whether the transformation produced a better model. 
...
[^1]: http://avesbiodiv.mncn.csic.es/estadistica/curso2011/boxcox.pdf
